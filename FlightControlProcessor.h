...
#if defined AltitudeHoldBaro
void processChangeHeightAltitudeCorrection()
{
  
  if (receiverCommand[AUX2] < 1470 && receiverCommand[AUX2] > 1530 || receiverCommand[MODE] < 1500 || receiverCommand[AUX1] < 1500) // If Not ID2, or Not Auto, or Alt Hold OFF 
      prev_hotel = 0.1;
  
  if (receiverCommand[AUX2] > 1470 && receiverCommand[AUX2] < 1530 && receiverCommand[MODE] < 1500 && receiverCommand[AUX1] > 1500) // If ID2, Not Auto, and Alt Hold ON
      baroAltitudeToHoldTarget -= 0.008;
       
  
  if ((Emergency[0] == 2 || Emergency[0] == 3) && receiverCommand[MODE] > 1500)
      Auto2or3 = true;
  else
      Auto2or3 = false;
     
  if (Emergency[0] == 1 && Landing[0] == 1 && receiverCommand[MODE] > 1500)
      AutoReturn = true;
  else
      AutoReturn = false;
      
  if (AutoReturn || Auto2or3) 
  {
    receiverCommand[AUX1] = 2000;  // For faster response and no lag from waiting for readPilot              
    Current_Height = getBaroAltitude();

    if ((abs(Current_Height - hotel) > 2 || prev_hotel != hotel) && !Ascend && !Descend)
    {   
    
      isAltitudeHoldInitialized = false;
      Start_Height = getBaroAltitude();
      baroAltitudeToHoldTarget = Start_Height;
      
        if (Start_Height < hotel){
          Ascend = true;
          Descend = false;}
        else{
          Descend = true;
   ...
   if (Ascend)
     if (baroAltitudeToHoldTarget < hotel)
        baroAltitudeToHoldTarget += 0.008;
     else                                                
        Ascend = false;
...
//      Serial.println(AutoReturn);
//      Serial.println(Auto2or3);
//      Serial.print("Ascend: ");
...
//      Serial.println(beta);
//      Serial.print("move: ");
//      Serial.println(move_commands[1]);
//      Serial.print("baroAltitudeToHoldTarget: ");
//      Serial.println(baroAltitudeToHoldTarget);
//      Serial.println("");   
 ...
 */
void processFlightControl(int alpha, int beta) {
  
  // ********************** Calculate Flight Error ***************************
  calculateFlightError();
  
  // ********************** Update Yaw ***************************************
  processHeading(alpha, beta);
  ...
  if (frameCounter % THROTTLE_ADJUST_TASK_SPEED == 0) {  // 50hz task
    ...
    // ********************** Process Auto-Descent or Change of Height  **************************
    #if defined AutoLanding
      processAutoLandingAltitudeCorrection();
    #endif
    
      processChangeHeightAltitudeCorrection();
